...
def experiment_text(plan, payload, mode, sections=False):
    ...
    body=[source, plan['coordinate_anchor']['instruction']]
    ...
    if plan.get('user_instruction','').strip():body.append('Additional direction: '+plan['user_instruction'].strip())
...
class H3Camera(StableEditor):
    ...
    RETURN_TYPES=StableEditor.RETURN_TYPES
    RETURN_NAMES=StableEditor.RETURN_NAMES
    OUTPUT_TOOLTIPS=StableEditor.OUTPUT_TOOLTIPS
    @classmethod
    def INPUT_TYPES(cls):
        data=super().INPUT_TYPES()
        data['optional']['experiment_mode']=(EXPERIMENT_MODES,{'default':'Off','tooltip':
            'PT: Off usa v19. Article compact combina texto curto e keyframes; Article numbers only testa números sem direção verbal por trecho. Mantenha seed, loop closure e instruction iguais; instruction vazio isola melhor a comparação. São hipóteses de prompt locais.\n'
            'EN: Off uses v19. Article compact combines short text and keyframes; Article numbers only tests numbers without per-segment verbal direction. Keep seed, loop closure and instruction identical; an empty instruction isolates the comparison. These are local prompt hypotheses.'})
        return data
    ...
    def run(self,camera_trajectory,profile,interpolation,instruction,subject_framing=None,minimax_format=None,reference_image=None,elevation_range=None,orbit_direction=None,subject_box=None,runtime_task=None,prompt_detail=None,frame_mode='Freeze Frame',source_fps=24.,freeze_index=0,ui_language='Português',loop_closure='auto',experiment_mode='Off'):
        ...
        result[3]+='\nExperimental: '+experiment_mode+'. '+('Off uses v19. Active experiments override minimax_format with compact text plus data (H3 sections still apply). No camera adapter or fal API is used.' if en else 'Off usa v19. Experimentos ativos substituem minimax_format por texto compacto com dados (as seções H3 continuam disponíveis). Não usa adaptador de câmera nem API fal.')
